import { Link } from 'react-router-dom'
import heroImg from '../assets/hero.png'

const FEATURES = [
  {
    title: 'Learning Gap Analysis',
    description: '커리큘럼 PDF와 평가 데이터를 비교하여 교육과 평가 사이의 갭을 자동 분석합니다.',
    icon: (
      <svg className="h-8 w-8 text-blue-600" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 113.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m5.231 13.481L15 17.25m-4.5-15H5.625c-.621 0-1.125.504-1.125 1.125v16.5c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Zm3.75 11.625a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
      </svg>
    ),
    to: '/analyze',
  },
  {
    title: 'Struggle Predictor',
    description: '퀴즈 성적 패턴과 출석률을 AI로 분석하여 낙오 위험 학생을 사전에 예측합니다.',
    icon: (
      <svg className="h-8 w-8 text-amber-500" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
      </svg>
    ),
    to: '/predict',
  },
  {
    title: 'Curriculum Optimizer',
    description: '갭 분석 결과를 바탕으로 최적화된 수업 계획과 시간 배분을 AI가 생성합니다.',
    icon: (
      <svg className="h-8 w-8 text-green-600" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0 0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906 59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702 0 0 1 7.74-3.342M6.75 15a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm0 0v-3.675A55.378 55.378 0 0 1 12 8.443m-7.007 11.55A5.981 5.981 0 0 0 6.75 15.75v-1.5" />
      </svg>
    ),
    to: '/optimize',
  },
]

export default function LandingPage() {
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero */}
      <section className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_50%,white_0%,transparent_60%)]" />
        </div>

        <div className="relative max-w-6xl mx-auto px-6 py-20 lg:py-28">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-blue-200 text-sm font-medium tracking-wide uppercase mb-4">
                AI-Powered Education Analytics
              </p>
              <h1 className="text-4xl lg:text-5xl font-extrabold text-white leading-tight">
                가르친 것과 배운 것 사이의
                <span className="block text-blue-200 mt-1">간격을 AI가 찾는다</span>
              </h1>
              <p className="text-blue-100 text-lg mt-6 leading-relaxed max-w-lg">
                CurriculumMind는 커리큘럼과 평가 데이터를 AI로 분석하여
                학습 갭을 발견하고, 위험 학생을 예측하며, 최적화된 수업 계획을 제안합니다.
              </p>
              <div className="flex gap-3 mt-8">
                <Link
                  to="/analyze"
                  className="px-6 py-3 bg-white text-blue-700 rounded-lg font-semibold text-sm hover:bg-blue-50 transition-colors shadow-lg shadow-blue-900/20"
                >
                  분석 시작하기
                </Link>
                <Link
                  to="/predict"
                  className="px-6 py-3 bg-blue-500/30 text-white rounded-lg font-semibold text-sm hover:bg-blue-500/40 transition-colors border border-blue-400/30"
                >
                  학생 예측 보기
                </Link>
              </div>
            </div>
            <div className="hidden lg:flex justify-center">
              <img
                src={heroImg}
                alt="CurriculumMind"
                className="w-80 h-80 object-contain drop-shadow-2xl"
              />
            </div>
          </div>
        </div>
      </section>

      {/* Features */}
      <section className="max-w-6xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          <h2 className="text-2xl font-bold text-gray-900">핵심 기능</h2>
          <p className="text-gray-500 mt-2">AI 기반 교육 분석의 세 가지 축</p>
        </div>

        <div className="grid md:grid-cols-3 gap-6">
          {FEATURES.map(({ title, description, icon, to }) => (
            <Link
              key={to}
              to={to}
              className="group bg-white rounded-xl border border-gray-200 p-6 hover:border-blue-300 hover:shadow-lg transition-all"
            >
              <div className="mb-4">{icon}</div>
              <h3 className="text-lg font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">
                {title}
              </h3>
              <p className="text-sm text-gray-500 mt-2 leading-relaxed">{description}</p>
              <span className="inline-block mt-4 text-sm font-medium text-blue-600 group-hover:underline">
                시작하기 &rarr;
              </span>
            </Link>
          ))}
        </div>
      </section>

      {/* Footer */}
      <footer className="border-t border-gray-200 bg-white">
        <div className="max-w-6xl mx-auto px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-center sm:text-left">
            <p className="text-sm font-semibold text-gray-900">CurriculumMind</p>
            <p className="text-xs text-gray-500 mt-1">AI-Powered Curriculum Gap Analysis</p>
          </div>
          <div className="text-xs text-gray-400">2026 AI Education Hackathon</div>
        </div>
      </footer>
    </div>
  )
}
